...
namespace SeasonsCare.Api.Controllers.HealthRecords
{
    ...
    public class TemperaturesController : ControllerBase
    {
        ...
        [EndpointDescription("取得指定照護群組底下的體溫紀錄列表。前端需在 path 帶入 careGroupId，通常來自照護群組列表或目前選取中的群組；可另外用 query string 傳入 page、pageSize、sort。")]
        ...
        [ProducesResponseType(StatusCodes.Status403Forbidden)]
        [EndpointSummary("依照 recordId 取得單筆體溫紀錄")]
        [EndpointDescription("依照 path 參數 careGroupId 與 recordId 取得單筆體溫紀錄。前端通常會先呼叫體溫列表 API，再把回傳資料中的 id 當作 recordId 帶入；若該資料不屬於目前 careGroupId，會回傳 404。")]
        ...
        [EndpointDescription("在指定照護群組底下建立新的體溫紀錄。前端需在 path 帶入 careGroupId，並在 request body 提供 value；notes 與 recordDate 可依需求填寫。")]
        ...
        [EndpointDescription("更新指定的體溫紀錄。前端需在 path 帶入 careGroupId 與 recordId，並在 request body 提供要更新的欄位與 updatedAt；updatedAt 應來自先前查詢單筆或列表 API 回傳的資料，用於樂觀鎖檢查。")]
        ...
        [EndpointDescription("刪除指定的體溫紀錄。前端需在 path 帶入 careGroupId 與 recordId；這兩個值通常來自體溫列表或單筆查詢結果。此操作為 soft delete，不會物理刪除資料。")]
