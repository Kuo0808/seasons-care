...
public class WeightsTenantIsolationIntegrationTests
{
    [Fact]
    public async Task GetRecords_ReturnsOnlyRecordsFromRequestedCareGroup()
    {
        ...
        await factory.SeedAsync(
            ...
            SeedDataHelper.CreateWeight(careGroupA.Id, 60.4m),
            SeedDataHelper.CreateWeight(careGroupB.Id, 72.8m));

        var response = await client.GetAsync($"/api/care-groups/{careGroupA.Id}/health-records/weights");
        ...
        Assert.Equal(60.4m, items[0].GetProperty("value").GetDecimal());
    }

    [Fact]
    public async Task CreateRecord_PersistsWeight_InRequestedCareGroup()
    {
        using var factory = new RealApiFactory();
        using var client = factory.Factory.CreateClient();
        ...
        var response = await client.PostAsJsonAsync($"/api/care-groups/{careGroup.Id}/health-records/weights", new
        {
            value = 61.5,
            notes = "after lunch"
        });
        ...
        var saved = await factory.FindAsync<WeightRecord>(createdId);
    ...
    public async Task GetRecordById_ReturnsNotFound_WhenRecordIsInDifferentCareGroup()
    {
        ...
        var careGroupA = SeedDataHelper.CreateCareGroup("Group A");
        var careGroupB = SeedDataHelper.CreateCareGroup("Group B");
        var recordInGroupB = SeedDataHelper.CreateWeight(careGroupB.Id, 68.2m);
        ...
        var response = await client.GetAsync($"/api/care-groups/{careGroupA.Id}/health-records/weights/{recordInGroupB.Id}");
    ...
    public async Task UpdateRecord_RefreshesUpdatedAt_AndPersistsChanges()
    {
        ...
        var careGroup = SeedDataHelper.CreateCareGroup("Group A");
        var existing = SeedDataHelper.CreateWeight(careGroup.Id, 59.8m);
        await factory.SeedAsync(
            SeedDataHelper.CreateUser(),
            careGroup,
            SeedDataHelper.CreateMember(careGroup.Id),
            existing);
        var persisted = await factory.FindAsync<WeightRecord>(existing.Id);

        var response = await client.PutAsJsonAsync($"/api/care-groups/{careGroup.Id}/health-records/weights/{existing.Id}", new
        {
            value = 60.2,
            notes = "updated",
        ...
        var saved = await factory.FindAsync<WeightRecord>(existing.Id);
        Assert.NotNull(saved);
        Assert.Equal(60.2m, saved!.Value);
    ...
    public async Task DeleteRecord_SoftDeletesRecord_AndHidesItFromList()
    {
        ...
        var careGroup = SeedDataHelper.CreateCareGroup("Group A");
        var existing = SeedDataHelper.CreateWeight(careGroup.Id, 62.1m);
        await factory.SeedAsync(
            SeedDataHelper.CreateUser(),
            careGroup,
            SeedDataHelper.CreateMember(careGroup.Id),
        ...
        var deleteResponse = await client.DeleteAsync($"/api/care-groups/{careGroup.Id}/health-records/weights/{existing.Id}");
        Assert.Equal(HttpStatusCode.OK, deleteResponse.StatusCode);

        var allRecords = await factory.GetWeightsAsync();
        var deleted = allRecords.Single(x => x.Id == existing.Id);
        Assert.NotNull(deleted.DeletedAt);

        var listResponse = await client.GetAsync($"/api/care-groups/{careGroup.Id}/health-records/weights");
