...
public class WeightServiceTests
{
    ...
    {
        var repository = new FakeWeightRepository();
        var groupRepository = new FakeCareGroupRepository(isMember: false);
        var service = new WeightService(repository, groupRepository);

        var exception = await Assert.ThrowsAsync<DomainException>(() =>
            service.CreateRecordAsync(Guid.NewGuid(), Guid.NewGuid(), new CreateWeightRequest
            {
                Value = 60m
            }));
    ...
    public async Task UpdateRecordAsync_ThrowsConflict_WhenUpdatedAtDoesNotMatch()
    {
        ...
        var repository = new FakeWeightRepository(existing);
        var groupRepository = new FakeCareGroupRepository(isMember: true);
        var service = new WeightService(repository, groupRepository);

        var exception = await Assert.ThrowsAsync<DomainException>(() =>
            service.UpdateRecordAsync(Guid.NewGuid(), existing.CareGroupId, existing.Id, new UpdateWeightRequest
            {
                Value = 61.5m,
    ...
    public async Task CreateRecordAsync_CreatesRecord_WithInitialConcurrencyTimestamp()
    {
        ...
        var result = await service.CreateRecordAsync(userId, careGroupId, new CreateWeightRequest
        {
            Value = 58.4m
        });
    ...
    private sealed class FakeWeightRepository : IWeightRepository
    {
        public List<WeightRecord> Items { get; } = new();

        public FakeWeightRepository(params WeightRecord[] items)
        {
            Items.AddRange(items);
        }

        public Task<PagedResponse<WeightRecord>> GetPagedAsync(Guid careGroupId, PaginationRequest request)
        {
        ...
        public Task<WeightRecord> AddAsync(WeightRecord record)
        {
            Items.Add(record);
            return Task.FromResult(record);
        }

        public Task<WeightRecord> UpdateAsync(WeightRecord record)
        {
            return Task.FromResult(record);
        }
    }
